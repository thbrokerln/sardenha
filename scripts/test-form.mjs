...
await p.evaluate(() => document.querySelector('[data-cta="fork-veraneio"]').click());
...
await p.select("#andar", "Andar alto — pela vista");
await p.evaluate(() => document.getElementById("leadForm").requestSubmit());
await sleep(600);
...
linha(perfilDepois === "Veraneio — casa de praia própria", `CTA pre-seleciona objetivo -> "${perfilDepois}"`);
...
linha(!/\(\d{2}\)\s?\d/.test(texto) && !texto.includes("Meu WhatsApp"),
      "mensagem NAO repete o telefone (ele ja esta no cabecalho da conversa)");
linha(texto.toLowerCase().includes("andar alto"), "mensagem leva a preferencia de andar");
linha(/tabela|dispon/i.test(texto), "mensagem termina com pedido concreto");
linha(texto.includes("43m²"), "mensagem leva a unidade escolhida");
linha(texto.toLowerCase().includes("veraneio"), "mensagem leva o objetivo");
...
if (!req) {
  ...
  linha(!!req.corpo?.andar, `andar gravado = "${req.corpo?.andar}"`);
